...
extern double RunsDistribution_p_Value;
extern double RunsDistribution_v_Value;
...
int RunsDistribution(int n, BitSequence *epsilon)
{
    ...
    bi = (double *)calloc(sizeof(double), (unsigned int)n);
    if (NULL == bi) {
        goto end;
    }
    gi = (double *)calloc(sizeof(double), (unsigned int)n);
    if (NULL == gi) {
        goto end;
    }
    e = (double *)calloc(sizeof(double), (unsigned int)n);
    if (NULL == e) {
        goto end;
    }
    ...
    V = sum_bi + sum_gi;
    RunsDistribution_v_Value=V;

    p_value = cephes_igamc(k - 1, V / 2);
    RunsDistribution_p_Value=p_value;
    if (p_value < ALPHA) {
        goto end;
    }
