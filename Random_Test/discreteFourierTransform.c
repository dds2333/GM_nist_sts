...
extern double DiscreteFourierTransform_p_Value;
extern double DiscreteFourierTransform_v_Value;
...
{
    double	p_value, upperBound, percentile, N_l, N_o, d,*X;
	double* m = NULL;
	double* wsave = NULL;
    int i, count, ifac[15];
...
//			fprintf(stats[7],"\t\tUnable to allocate working arrays for the DFT.\n");
    ...
    DiscreteFourierTransform_v_Value=d;
    p_value = nist_erfc(fabs(d)/sqrt(2.0));
    DiscreteFourierTransform_p_Value=p_value;
