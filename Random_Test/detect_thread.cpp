...
{
    //
    for(int i=0;i<MainWindow::m_bit_Len.count()/2;i++)
    {
        ...
        data.ret=detect_fun_thread(MainWindow::m_rand_bits[i],MainWindow::m_bit_Len[i]);
        data.f_Name=MainWindow::f_name[i];

        if(i==MainWindow::m_bit_Len.count()/2-1)
        {
            data.finish=true;
        }
        emit Send2UI(data);
...
//检测函数
int detect_thread::detect_fun_thread(BitSequence* bits,int len)
{
    //统计0和1的个数
    int num_0=0;
    int num_1=0;
    ...
    if(checkbox9_ischeck==true)
    {
        ...
        else
        {
            ...
            QString p_val=QString::number(SelfCorrelation_p_Value,'f',6);
            p_val="PASS  p_value = "+p_val;
            QString v_val=QString::number(SelfCorrelation_v_Value);
            v_val="  统计值 = "+v_val;
            QString val=p_val+v_val;
            val+="  耗时:  ";
            val+=time_C_8;
